...
void USBDeviceManager::device_add(libusb_device *dev){
    ...
    if (current_config != devdesc.bNumConfigurations) {
        ...
        retcustomassure(MUXException_device_disconnected,!(err = libusb_set_configuration(handle, devdesc.bNumConfigurations)), "Could not set configuration %d for device %d-%d: 0x%08x", devdesc.bNumConfigurations, bus, address, err);
    }
